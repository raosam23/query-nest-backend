...
This module provides the writer_agent function, which uses an LLM to generate
a structured, organized, and readable report based on a query, key claims,
and a fact check report.
"""
...
async def writer_agent(state: ResearchState) -> dict:
    ...
    if not key_claims or not fact_check_report:
        await update_agent_log(
            db_session,
            log.id,
            "Insufficient information to generate a report",
            AgentStatus.FAILED,
        )
    ...
    try:
        ...
        if not response:
            await update_agent_log(
                db_session, log.id, "Failed to generate report", AgentStatus.FAILED
            )
            return {"final_report": "Failed to generate report"}
        await update_agent_log(
            db_session,
            log.id,
            f"Report generated successfully. Length: {len(response.content)} characters",
        )
